...
import { skillList, MasteryRank, masteryBonus } from '../types/character';

type Skills = Record<string, { mastery: MasteryRank; bonusEquip: number }>;
interface SkillsSectionProps {
    niveau: number;
    stats: Record<string, number>;
    skills: Skills;
    onChange: (key: string, value: Partial<{ mastery: MasteryRank; bonusEquip: number }>) => void;
...
export function SkillsSection({ niveau, stats, skills, onChange, onRoll }: SkillsSectionProps) {
    ...
    return (
        <View>
            <View style={styles.tableHeader}>
                <Text style={[styles.headerCell, { width: widths.label, textAlign: 'left' }]}>Compétence</Text>
            ...
            {skillList.map(({ key, label, carac }) => {
                const skill = skills[key] ?? { mastery: "inexpérimenté", bonusEquip: 0 };
                const mastery: MasteryRank = masteryRanks.includes(skill.mastery) ? skill.mastery : "inexpérimenté";
                const equip = Number.isFinite(skill.bonusEquip) ? skill.bonusEquip : 0;
